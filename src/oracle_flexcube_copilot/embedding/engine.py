...
class EmbeddingEngine:
    ...
    def _embed_batch_with_retry(self, texts: list[str]) -> list[list[float]]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.warning(
                    "Ollama connection/timeout error: %s (attempt %d/%d)",
                    e,
                    attempt + 1,
                    self.max_retries,
                )
    ...
    def embed_chunks(self, chunks: list[Chunk]) -> tuple[list[EmbeddedChunk], EmbeddingMetrics]:
            ...
            try:
                ...
                for chunk, vector in zip(batch, vectors):
                    ...
                    embedded_chunks.append(
                        self._create_embedded_chunk(chunk, vector, batch_latency)
                    )
        ...
        if total_success > 0:
            ...
            metrics.vectors_per_second = (
                total_success / metrics.total_time_seconds
    ...
    def _create_embedded_chunk(
        self, chunk: Chunk, vector: list[float], latency: float
    ) -> EmbeddedChunk:
