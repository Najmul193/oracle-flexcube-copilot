...
class TestEmbeddingEngine:
    ...
    def test_successful_batch_embedding(
        self, mock_client: MagicMock, dummy_chunks: list[Chunk], tmp_path
    ) -> None:
    ...
    def test_failure_after_max_retries(
        ...
        engine = EmbeddingEngine(
            client=mock_client, cache=cache, batch_size=3, max_retries=2, base_backoff=0.01
        )
        embedded, metrics = engine.embed_chunks(dummy_chunks)
